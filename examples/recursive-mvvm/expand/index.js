...
const viewModel = TemplateEngine.reactive({
    ...
    reverseTransform(personViewModelItem, reversedViewModelProps = ['id', 'name', 'wage', 'age', 'street', 'city', 'children'], modelItem) {
        const reversedId = ModelSynchronization.getReversedViewModelPropValue(personViewModelItem, reversedViewModelProps, 'id')
        const reversedName = ModelSynchronization.getReversedViewModelPropValue(personViewModelItem, reversedViewModelProps, 'name')
        const reversedWage = ModelSynchronization.getReversedViewModelPropValue(personViewModelItem, reversedViewModelProps, 'wage')
        const reversedAge = ModelSynchronization.getReversedViewModelPropValue(personViewModelItem, reversedViewModelProps, 'age')

        return {
            id: reversedId,
            name: reversedName,
            wage: reversedWage !== undefined ? reversedWage.slice(0, -4) : undefined,
            birthyear: reversedAge !== undefined ? new Date().getFullYear() - reversedAge : undefined,
