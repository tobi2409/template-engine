...
export function handleMapInputAttribute(cloned, attr, data, contextStack, params, dependencies = {}) {
    try {
        // map-{event}-{property}="mapperFunctionName"
        const parts = attr.name.split('-')
        const event = parts[1] // e.g., 'input', 'change'
        const property = parts[2] // e.g., 'value'
        const mapperName = attr.value
        
        const resolvedMapper = resolveEx(mapperName, data, contextStack, params)

        if (typeof resolvedMapper.value !== 'function') {
            throw new Error(`[TemplateEngine] map-input mapper "${mapperName}" must be a function`)
        }
        
        // Add event listener for mapped input (UI → Data with transformation)
        cloned.addEventListener(event, (e) => {
            const inputValue = e.target[property]
            // Call mapper function with value and context
            const context = {
                event: e,
                element: cloned,
            ...
            resolvedMapper.value.call(data, inputValue, context)
            
            // Trigger refreshes for dependent keys
            // Note: The mapper function is responsible for updating the source data,
            // which will trigger Proxy setters and dependency notifications
        })
        
        cloned.removeAttribute(attr.name)
    } catch (error) {
        throw new Error(`[TemplateEngine] Error handling map-input attribute "${attr.name}": ${error.message}`)
    }
}
